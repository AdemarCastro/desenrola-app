...
export class AuthService {
  public async login(loginData: LoginInputDto): Promise<{ token: string }> {
    const { email, senha } = loginData;

    const usuario = await UsuarioService.findUsuarioByEmailForAuth(email);
  ...
  public async register(registerData: RegisterInputDto): Promise<{ id: number; email: string }> {
    const { email, senha, primeiro_nome, sobrenome, data_nascimento } = registerData;

    const usuarioExistente = await UsuarioService.findUsuarioByEmailForAuth(email);
    if (usuarioExistente) {
